# delete CI IG RT SN VCN
# usage compartment_id teardown
# only teardown branch is actual
...
def teardown_resources(oci_cfg, compartment_id, name):
    ...
    print("teardown_resources name  " + name + ' vcn_name(name) ' + vcn_name(name))

    # Collect the OCI resources to delete.
    vcn = ig = rt = sn = sg = fn = None
    config = oci.config.from_file()
    identity_client = oci.identity.IdentityClient(config)
    list_network_sources_response = network_client.list_vcns(compartment_id)
    ...
    print(list_network_sources_response.data)

    #vcn = get_unique_vcn_by_name(network_client, compartment_id, vcn_name(name))

    vcn = list_network_sources_response.data[0]
    ComputeClient = oci.core.ComputeClient(config)

    cil = oci.pagination.list_call_get_all_results(ComputeClient.list_instances, compartment_id=compartment_id).data
    if len(cil) != 0:
        for ci in cil:
        #ci = cil[0]
            if ci is not None:
                print('Terminating Instance: {}'.format(ci.id))
                compute_client_composite_operations = oci.core.ComputeClientCompositeOperations(ComputeClient)
                ...
                print('Terminated Instance: {}'.format(ci.id))
                print()

    if vcn is not None:
        #ig = get_unique_ig_by_name(
        #    network_client, compartment_id, vcn.id)
        igl = network_client.list_internet_gateways(compartment_id).data
        if len(igl) != 0:
            ig = igl[0]
        #rt = get_unique_route_table_by_name(
        #    network_client, compartment_id, vcn.id)
        rt  = network_client.list_route_tables(compartment_id).data[0]
        #sn = get_unique_subnet_by_name(
        #    network_client, compartment_id, vcn.id)
        snl = network_client.list_subnets(compartment_id).data
        if len(snl) != 0:
            sn = snl[0]

        sgl = network_client.list_network_security_groups(compartment_id = compartment_id).data
        if len(sgl) != 0:
            sg = sgl[0]
    print("ig")
    print(ig)
    ...
    if sg is not None:
        vn_composite_ops.delete_network_security_group_and_wait_for_state(
            sg.id, wait_for_states=[oci.core.models.NetworkSecurityGroup.LIFECYCLE_STATE_TERMINATED]
        )
        print('Delete Security group: {}'.format(sg.id))
